# Wavvy — Project Identity

Version: 1.0
Last Updated: 2026-03-03
Purpose: 프로젝트 전략/브랜드 정의 (What & Why)

---

## 1. What is Wavvy

**한국어 가사 음악 플레이리스트 채널**

> "하루는 감정의 지도다."
> 시간은 가장 보편적인 감정 트리거.

청자의 현재 시간에 맞는 음악, "지금 이 순간"을 대변하는 한국어 가사.

---

## 2. Core Differentiator

| 기존 플레이리스트 | Wavvy |
|------------------|-------|
| 영어/Lo-fi 중심 | **한국어 가사** |
| 배경음악 | **의미 있는 가사** (장르별 밀도 조절) |
| 장르 기반 | **시간 기반** (24H Station) |

---

## 3. Brand DNA (Constants)

```
• 한국어 가사 only
• Chest-dominant vocal (진성 기반)
• Single lead, no harmonies
• Chorus = arrangement expansion only
• 금지: Airy, Falsetto, Harmonized, Backing vocals
```

---

## 4. 24H Station Concept

11개 시간대 = 11개 감정 스테이션

| Time | Station | Tone |
|------|---------|------|
| 02:00 | 새벽잠 | 불면, 고요 |
| 04:00 | 여명 | 고독, 여백 |
| 07:00 | 기상 | 시작, 담담 |
| 10:00 | 업무시간 | 지속, 루틴 |
| 14:00 | 오후햇살 | 나른, 따뜻 |
| 18:30 | 퇴근길 | 해방, 전환 |
| 21:00 | 밤산책 | 고요, 사색 |
| 23:30 | 불면 | 무게, 고백 |

상세 설정은 각 `SERIES/[시리즈]/concept.md` 참조.

---

## 5. Lyric Philosophy

**금지:**
- 설명적 문장
- 감정 직접 표출 (사랑해, 슬퍼)
- 메타 표현

**지향:**
- 사물·공간·현상 중심
- 한 문장 한 이미지
- "가사 없는 듯 들리지만 읽으면 남는 가사"

---

## 6. Series Mapping

| Series | Time | Genre |
|--------|------|-------|
| 04-00 | 04:00 | Slow R&B |
| 06-00 | 06:00 | Fast Lo-fi |
| 10-00 | 10:00 | Chillhop |
| 14-00 | 14:00 | Soft R&B |
| 16-00 | 16:00 | 감성 락 |
| 18-00 | 18:00 | Neo-Soul |
| 21-00 | 21:00 | City Pop |

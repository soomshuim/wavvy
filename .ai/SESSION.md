...
| 작업 | 날짜 |
|------|------|
| fix: Track 01 구조 수정 (Hook 축소, 인스트 확장, 보컬 믹스 다운) | 2026-03-06 |
...
- 루프 기반 구조 ([intro]→[loop A]→[loop B]→[loop A']→[outro])
- 10곡 → **15곡 확장** (Suno 출력 길이 짧아서)
...
- 맥락이 즉시 잡히는 구체적 미시관찰 선호
- 대화체/일상적 한국어 > 문학적/시적 표현
- "꺼져" 같은 욕설 동음이의어 주의

---

## 채널 브랜딩 확정 (2026-03-03)
...
### TODO
- [ ] Wavvy 로고 핸드라이팅 애니메이션 완성 (Canva Pro)
...
- [x] **SSOT 책임/우선순위 충돌 정리**
  - `_INDEX.md` 책임 매트릭스에 작업 절차 권한을 `MANAGER.md`로 명시
  - `MANAGER.md`에 Phase 0.5(`txt → 컨펌 → concept`) 절차를 SSOT로 승격
  - `CLAUDE.md`는 요약/실행 매뉴얼로 정렬 (비-SSOT 참조 문서)

- [x] **가사 입력 규칙 최신화**
  - `[]` 구조 태그 / `()` 구조 직후 1행 보컬·악기·코드 메타 규칙으로 통일
  - `[instrumental]`을 필수에서 조건부로 조정

- [x] **Style Prompt 길이 정책 통일**
  - 기준: `<= 900자 (공백 포함 문자)`
  - `STYLE.md`를 길이 규칙 SSOT로 고정하고 참조 문서 동기화

- [x] **문서 메타 포맷 통일**
  - 헤더 `Version / Last Updated / Purpose` 포맷 정규화
  - 핵심 문서 `Last Updated`를 `2026-02-16`으로 동기화
...
- [x] **Track 01 "문" 최종 가사 반영**
  - V1: "숨을 한 번 내쉬어" → "편한 신발을 신고", "열쇠 돌려" → "손잡이 돌려"
  - Pre-Chorus: "문 하나 사이에 낮과 밤이 갈려" → "문틈 사이로 빛이 새어들어"
- [x] **Track 01 "문" 최종 스타일 반영**
  - Articulation 선두 배치, 콤마 구분 단일 블록 포맷
- [x] **concept.md 업데이트**
  - 키워드 축: 열쇠 → 손잡이, 훅/상태 추가
...
- [x] **PM_0900 시리즈 컨셉 확정**
  ...
  - 레퍼런스: 죠지(바라봐줘요/오랜만에), 김현철(Drive/달의몰락), 유키카(NEON), 아소토유니온(Think About Chu)

- [x] **10곡 트랙 리스트 완성**
  | # | 제목 | 장소 | 장르 | BPM | Key | Vocal |
  |---|------|------|------|-----|-----|-------|
  | 01 | 문 | 현관 | 발라드 | 75 | Db | Male |
  | 02 | 담 | 골목 | 네오소울 | 88 | Eb | Female |
  | 03 | 불빛 | 편의점 | 시티팝 | 108 | F | Female |
  | 04 | 벤치 | 공원 | 발라드 | 72 | Ab | Male |
  | 05 | 물결 | 강변 | 시티팝 | 115 | A | Male |
  | 06 | 건넘 | 다리 | 네오소울 | 92 | Eb | Female |
  | 07 | 막차 | 정류장 | 발라드 | 68 | Db | Female |
  | 08 | 창가 | 카페 | 네오소울 | 85 | Db | Male |
  | 09 | 시야 | 옥상 | 시티팝 | 112 | G | Male |
  | 10 | 집 | 귀가 | 발라드 | 70 | Db | Female |

- [x] **수미상관 구조 완성**
  - Track 01 "문": 문 열림, 출발, 설렘 (Db Major)
  - Track 10 "집": 문 닫힘, 도착, 안도 (Db Major)

- [x] **파일 생성 완료**
  - `SERIES/PM_0900/concept.md`
  - `track01~10_lyrics.txt` (10개)
  - `track01~10_style.txt` (10개, 모두 800자 내외)
  - `track01~10_exclude.txt` (10개)
...
- [x] **PM_0400 Track 09 "서랍" Shorts 제작** (no commit - no git)
  - 구간: 0:50 ~ 1:50, 60초, 타이틀 없음
  - 출력: `SERIES/PM_0400/output/shorts/short_서랍.mp4`
- [x] **골목 숏츠 YouTube 메타데이터 문서화** (no commit - no git)
  - `SERIES/Single/골목_youtube.md`에 Shorts 섹션 추가
  - 제목: "퇴근길의 위로 | 골목 - soomshuim"
  - 고정댓글: PM 06:00 Track 07, 풀 버전 링크 포함
- [x] **시리즈 폴더 네이밍 24h→12h 통일** (no commit - no git)
  - PM_1400→PM_0200, PM_1600→PM_0400, PM_1800→PM_0600
  - 폴더명 + 파일 내 참조 + CLAUDE.md 구조 섹션 수정

### 2026-02-01 - PM_0400 시리즈 완성

- [x] **PM_0400 시리즈 패키징 및 업로드 준비 완료** `2a64214`
  - 9트랙 x 2회 반복 = 58.6분 영상 생성
  - description 최종 버전 (GPT 협업)
  - 비공개 업로드 완료 (24시간 알고리즘 학습 대기)

- [x] **Track 09 "서랍" 완성 - 시리즈 피날레** `87a853b`
  - 퍼소나: 모두 (시리즈 마무리)
...
### 2026-01-31 - PM_0400 Track 05 "기다림" 완성
...
### 2026-01-31 - PM_0400 Track 04 스타일 수정 + claude-center 설정
...
### 2026-01-31 - PM_0400 Track 04 "공강" 초안 완성
...
### 2026-01-31 - PM_0400 Track 02 "눈치" 확정 + 전체 플리 구조 확정
...
### 2026-01-31 - PM_0400 Track 01 "가도 되나요" 확정
...
- [x] **concept.md 통합**
  - SERIES/PM_0400/concept.md 생성
  - track01_style.txt, track01_lyrics.txt 삭제

### 2026-01-30 - PM_0400 "막판뒤집기" 시리즈 시작
...
### 2026-01-29 - PM_0600 유튜브 업로드 준비 `d5d0e16`

- [x] **draft_description.txt 작성**
  - 제목: `[Playlist] [PM 06:00] soomshuim | 오늘도 수고했어요, 집까지 함께할게요`
  - 설명: 퇴근길 맥락 반영 (두근거림 → 따뜻함, 응원)
  - 고정 댓글: 채널 기본 포맷 적용
  - final.mp4 압축: 2.2GB → 1.2GB (CRF 26)

### 2026-01-29 - PM_0600 9트랙 시리즈 완성 `446051b`
...
### 2026-01-29 - PM_0600 Track 05-06 완성
...
### 2026-01-29 - PM_0600 Track 04 차창밖 `217fe40` `4401d5a`
...
### 2026-01-28 - PM_0600 퇴근길 시리즈 `216aee5` `6a1a4ba`

- [x] **PM_0600 "퇴근길" 시리즈 추가**
...
- [x] **PM_0200 트랙 순서 재배치 (A안)** `a01b253`
  - 기존: 산책(01)이 오프닝
...
- [x] **PM_0200 vol.2 시드 트랙 설계** `c6ffe3a`
...
- **vol.2 (PM_0200)**: 진행 중
  ...
  - **수미상관 완성**: 멍(01) ↔ 깸(10)
  - 작업 디렉토리: `SERIES/PM_0200/`
  - Suno 생성 대기
- **PM_0400 "4시의 사람들"**: Track 01-08 완성, 전체 구조 확정

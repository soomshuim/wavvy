...
### 레퍼런스 DNA 미반영 (PM_0400 Track 01 실패)
...
### 불충분한 웹 리서치 (PM_0400 Track 09 실패)

**날짜**: 2026-02-01

**상황**: 다비치 "타임캡슐" 시드곡 리서치 시 악보만 분석하고 웹 리서치 생략
...
❌ BPM 추정치만 제시 (검증 없음)
❌ 작곡가 정보, 프로듀싱 스타일 누락
❌ 곡 배경/의도/차트 성적 등 컨텍스트 없음
```

**원인**:
- "악보가 있으니 충분하다"는 착각
- 웹 리서치 없이 악보 기반 추론만 진행
- 환상효과(Hallucination) 검증 없음

**리스크**:
- BPM, 장르, 프로듀싱 특징 등 잘못된 정보 기반 설계
- 작곡가 의도와 다른 방향으로 곡 설계
- 원곡 DNA 핵심 요소 누락
...
```
┌─────────────────────────────────────────────────────────┐
│  Seed Research Workflow v1.0                            │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Phase 1: 웹 리서치 (필수)                              │
│  ────────────────────────                               │
│  □ WebSearch 최소 3회 이상 (다른 키워드로)              │
│    - "[곡명] [아티스트] 분석"                           │
│    - "[곡명] BPM 템포 키"                               │
│    - "[작곡가] 프로듀싱 스타일"                         │
│  □ WebFetch 최소 5개 소스                               │
...
│  □ 코드 진행 추출                                       │
│  □ 멜로디 특징 분석                                     │
│  □ 구조 분석 (Intro-Verse-Chorus-Bridge-Outro)         │
│                                                         │
│  Phase 3: 정보 교차 검증 (QC)                           │
│  ────────────────────────                               │
│  □ BPM: 최소 2개 소스에서 일치 확인                     │
│  □ Key: 악보 ↔ 웹 정보 일치 확인                        │
│  □ 작곡가/프로듀서 정보 정확성 확인                     │
│  □ 발매일/앨범 정보 확인                                │
│  □ 환상효과 체크: "이 정보의 출처가 있는가?"            │
│                                                         │
│  Phase 4: 리포트 작성                                   │
│  ────────────────────────                               │
│  □ Sources 섹션에 모든 참조 URL 기재                    │
│  □ 확인된 정보 vs 추정 정보 구분 표기                   │
│  □ PM_0400 적용 포인트 명시                             │
│                                                         │
│  ⚠️ QC FAIL 조건 (하나라도 해당 시 재진행)              │
│  ────────────────────────                               │
│  - 웹 소스 10개 미만                                    │
│  - BPM/Key 교차 검증 실패                               │
│  - 출처 없는 정보 포함                                  │
...
```markdown
## [곡명] Seed Research Report

### 1. Basic Info (웹 검증 완료)
| 항목 | 값 | 출처 |
|------|-----|------|
| 발매일 | YYYY-MM-DD | [출처] |
| Key | X Major/minor | 악보 + [출처] |
| BPM | NNN | [출처1], [출처2] |
| 작사/작곡 | OOO | [출처] |
...
## 비디오 크로스페이드 이슈 (2026-03-01)

### 문제
- 사용자가 "비디오 크로스페이드" 요청
- Claude가 "오디오 크로스페이드"로 오해하고 계속 잘못된 방향으로 작업
- 사용자가 여러 번 정정해도 같은 실수 반복
- 결과: 사용자가 잘못된 영상으로 유튜브 업로드 후 삭제, 시간/토큰 낭비

### 원인
- `vibem.py pack`이 오디오 크로스페이드만 적용하고 비디오는 `-stream_loop`로 단순 반복
- Claude가 이 차이를 인지하지 못함
- 사용자 요청을 정확히 파악하지 않고 추측으로 작업

### 재발방지
1. "크로스페이드" 요청 시 오디오/비디오 구분 필수 확인
2. `vibem.py pack` = 오디오만. 비디오 크로스페이드는 별도 FFmpeg xfade 필요
3. 대규모 작업 전 테스트 먼저 (3회 반복 등)
4. 기존에 만든 파일 (예: loop_5x_xfade.mp4) 재활용

### 수정된 문서
- `CLAUDE.md` Hard Constraints #5 추가
- `CLAUDE.md` Workflow Rules > 영상 패키징 워크플로우 추가
- `~/.claude/projects/-Users-zen/memory/MEMORY.md` 크로스페이드 규칙 추가

### 워크플로우 확정
1. 사용자가 테스트 비디오 크로스페이드 제작
